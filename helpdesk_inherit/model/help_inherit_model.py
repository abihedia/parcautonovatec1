...
class HelpdeskModelHerit(models.Model):
    ...
    products_id = fields.Many2one('product.product', string='Article', domain="[('id', 'in', [ a['fleet_artic_id'][0] for a in self.env['fleet.vehicle'].search_read([('partner_id', '=', self.partner_id.id)],['fleet_artic_id'])])]")
